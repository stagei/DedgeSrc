...
SqlMmdConverter can bundle Python and Node.js runtimes directly in the NuGet package, making it completely self-contained with zero manual setup required by users. This document outlines the strategies, tools, and implementation details.
...
```
SqlMmdConverter.1.0.0.nupkg (Platform-specific)
├── lib/
│   └── net10.0/
│       └── SqlMmdConverter.dll
├── runtimes/
...
│       ├── native/
│       │   └── SqlMmdConverter.dll (runtime-specific)
│       ├── python/
...
│           └── mmd_to_sql.js
└── build/
    └── SqlMmdConverter.targets (auto-copy runtimes)
```
...
<Project Sdk="Microsoft.NET.Sdk">
  ...
  <ItemGroup>
    <None Include="build\SqlMmdConverter.targets">
      <Pack>true</Pack>
...
```xml
<!-- build/SqlMmdConverter.targets -->
<Project>
  <Target Name="CopySqlMmdConverterRuntimes" BeforeTargets="Build">
    <ItemGroup>
...
Write-Host "Building SqlMmdConverter for $RuntimeId" -ForegroundColor Green

# Create runtime directories
$runtimeDir = "src/SqlMmdConverter/runtimes/$RuntimeId"
...
```bash
SqlMmdConverter.win-x64.1.0.0.nupkg      (~50 MB)
SqlMmdConverter.linux-x64.1.0.0.nupkg    (~55 MB)
SqlMmdConverter.osx-x64.1.0.0.nupkg      (~55 MB)
```

### Option 2: Meta-Package with Dependencies
```xml
<!-- SqlMmdConverter.nuspec -->
<package>
  <metadata>
    <id>SqlMmdConverter</id>
    <version>1.0.0</version>
    <dependencies>
      <group targetFramework="net10.0">
        <dependency id="SqlMmdConverter.win-x64" version="1.0.0" 
                    condition="$([MSBuild]::IsOSPlatform('Windows'))" />
        <dependency id="SqlMmdConverter.linux-x64" version="1.0.0" 
                    condition="$([MSBuild]::IsOSPlatform('Linux'))" />
        <dependency id="SqlMmdConverter.osx-x64" version="1.0.0" 
...
- ✅ **Zero manual setup** - users just `dotnet add package SqlMmdConverter`
