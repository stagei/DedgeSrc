...
function removeFilenameFromCleanPath {
    ...
    $extenstionArray = @(".TXT", ".LOG", ".BAT", ".%COMPUTERNAME%", ".TEST", ".AKT", ".CSV", ".ERR", ".BND",".CPY", ".DEL", ".DFT", ".EXE", ".FNK", ".GS", ".ICN", ".INT", ".JPG", ".PRN", ".REX", ".SQL")
    # $extenstionArray += @()
    ...
    # Check if last character is a backslash and remove it
    if ($cleanPath.EndsWith("\")) {
        $cleanPath = $cleanPath.Substring(0, $cleanPath.Length - 1)
    }

    #check if character 2..2 is a colon and character 3 is a backslash, replace all "\\" with "\"
    if ($cleanPath.Substring(1, 1) -eq ":" -and $cleanPath.Substring(2, 1) -eq "\") {
        $cleanPath = $cleanPath.Replace("\\", "\")
    }

    # check if the path starts "\\" and keep them, and  remove all other double backslashes 
    if ($cleanPath.StartsWith("\\\\")) {
        $cleanPath = "\\\\" + $cleanPath.Substring(2).Replace("\\\\", "\\")
    }
