...
**Category:** AdminTools  
**Status:** ✅ Activet  
**Deploy Target:** fkxprd-servere (*fkxprd*)  
**Complexity:** 🟢 Low  
...
### Problemstilling
DedgeCommon/Configfiles inneholder **CRITICALe konfigurasjonsfiler** for all Dedge-applikasjoner:
- Database connection strings
- Application settings (Applications.json, GlobalSettings.json)
...
- **Impact:** all applikasjoner slutter å fungere
- **Recovery:** Må gjenskape konfig manuelt (4-8 hours arbeid)
- **Kostnad:** Nedetid for 100+ brukere + gjenopprettingstid

**Eksempel from virkeligheten:**
- **Hendelse:** Admin endrer Applications.json, får syntax error
- **Resultat:** none applikasjoner kan starte
- **Uten Backup:** 6 hours downtime, manuell gjenoppretting
- **with Backup:** 5 minutes gjenoppretting from siste Backup
...
6. **Automatic cleanup** - none manuell vedlikehold

Dette gir **"Panic button recovery"** - rask gjenoppretting ved feil.

### Målgruppe
- **all administratorer** - Som endrer config files
- **Database-team** - Connection strings
- **DevOps** - Deployment Configurations
- **Support** - Quick recovery ved feil

### ROI/Gevinst
- ⏱️ **Tidssparing:** 6 hours/hendelse saved
- 🛡️ **Risiko-reduksjon:** Eliminerer config-tap
- 📊 **Compliance:** Config change history
- 🔄 **Automation:** 0 manuell innsats
- 💰 **Kostnad:** Eliminerer 2-3 hendelser/år (~18 hours)

**Eshourst årlig besparelse:** ~27 hours = ~40,500 NOK (basert på 1500 NOK/time)  
**Enda viktigere:** Risk av telt catastrofic downtime (Value: ubegrenset)
...
1. Finner DedgeCommon folder (X:\DedgeCommon or \\server\DedgeCommon)
2. Zipper Configfiles subfolder
3. Lagrer to: C:\opt\data\Backup-CommonConfigFiles\COMPUTERNAME_YYYYMMDD-HHMMSS.zip
...
│    └─> Write-LandMessage "JOB_STARTED" -Level JOB_STARTED   │
└─────────────────────────────────────────────────────────────┘
                           ↓
┌─────────────────────────────────────────────────────────────┐
...
│    └─> Write-LandMessage "Successfully zipped..." -Level INFO│
└─────────────────────────────────────────────────────────────┘
                           ↓
┌─────────────────────────────────────────────────────────────┐
...
│    │   └─> Write-LandMessage "Removed $file" -Level INFO     │
│    └─> Result: Only last 10 days kept                       │
└─────────────────────────────────────────────────────────────┘
                           ↓
┌─────────────────────────────────────────────────────────────┐
│ 7. Complete                                                  │
│    └─> Write-LandMessage "JOB_COMPLETED" -Level JOB_COMPLETED│
└─────────────────────────────────────────────────────────────┘
```
...
|--------|-----|--------------|-------------|
| Backup-CommonConfigFiles.ps1 | 30 | Low | Main Backup script |
| _deploy.ps1 | 4 | Low | Deployment script |
...
try {
    Write-LandMessage $(Get-InitScriptName) -Level JOB_STARTED
    ...
    Write-LandMessage "Successfully zipped $configfilesFolder to $zipFilePath" -Level INFO
    ...
    foreach ($file in $filesToRemove) {
        ...
        Write-LandMessage "Removed old Backup: $($file.Name)" -Level INFO
    }
    
    Write-LandMessage $(Get-InitScriptName) -Level JOB_COMPLETED
}
catch {
    Write-LandMessage "Error: $($_.Exception.Message)" -Level ERROR -Exception $_
    Write-LandMessage $(Get-InitScriptName) -Level JOB_FAILED
    Exit 9
}
```
...
│   └── Write-LandMessage "Backup-CommonConfigFiles.ps1" -Level JOB_STARTED
│       ├── Write to console
│       └── Write to land: C:\opt\data\AllPwshLand\dedge-server_20251103.land
│
├── Find-ExistingFolder -Name "DedgeCommon"
...
│   └── Write-LandMessage "Removed old Backup..." -Level INFO
│
└── Write-LandMessage $(Get-InitScriptName) -Level JOB_COMPLETED
```

#### Eksterne Avhengigheter
- ✅ **GlobalFunctions module** - Utoity functions
...
```powershell
# 1. all applikasjoner feiler ved oppstart
PS C:\> Get-App
