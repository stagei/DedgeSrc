if ($env:COMPUTERNAME.ToLower() -eq "t-no1fkxtst-db" -or $env:COMPUTERNAME.ToLower() -eq "p-no1fkxprd-db") {
    ...
    New-ScheduledTask -SourceFolder $PSScriptRoot\DedgeAuth-Backup-Database.ps1 -TaskFolder "Database" -RecreateTask $true -RunFrequency "Daily" -StartHour 2 -RunAsUser $true -RunAtOnce $true
