<#
.SYNOPSIS
    Restores the DedgeAuth PostgreSQL database from a backup using connection settings from DedgeAuth-DatabaseConfig.ps1.
...
$configPath = Join-Path $PSScriptRoot "DedgeAuth-DatabaseConfig.ps1"
if (-not (Test-Path $configPath -PathType Leaf)) {
    Write-LogMessage "DedgeAuth-DatabaseConfig.ps1 not found at: $($configPath)" -Level ERROR
    exit 1
}
