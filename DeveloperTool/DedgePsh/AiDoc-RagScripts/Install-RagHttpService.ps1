...
<#
.SYNOPSIS
    Installs the AiDoc RAG HTTP server as a Windows Service on port 8484.

.DESCRIPTION
    Creates and configures the AiDoc RAG HTTP service that:
    - Finds or creates a working Python venv in mcp-ai-docs\.venv.
...
.PARAMETER ServiceName
    Windows Service name (default: AiDocRag).
...
.PARAMETER AiDocRoot
    Root of the AiDoc folder. Default: parent of the folder containing this script.
...
function New-PythonVenv {
    ...
    $pipOut = & $venvPython -m pip install --upgrade pip 2>&1
    foreach ($line in $pipOut) { Write-LogMessage "pip-upgrade: $($line)" -Level INFO }
    Write-LogMessage "pip upgrade exit code: $($LASTEXITCODE)" -Level INFO
    ...
    if (Test-Path -LiteralPath $RequirementsFile) {
        ...
        $wheelsDir = Join-Path (Split-Path $RequirementsFile -Parent) 'offline_wheels'
        if (Test-Path -LiteralPath $wheelsDir) {
            Write-LogMessage "Offline wheels found at $($wheelsDir), using --no-index --find-links" -Level INFO
            $pipOut = & $venvPip install --no-index --find-links $wheelsDir -r $RequirementsFile 2>&1
        } else {
            Write-LogMessage "No offline wheels, trying online install" -Level INFO
...
try {
    ...
    $serverHttpPy = Join-Path $mcpDir "server_http.py"
    $requirementsTxt = Join-Path $mcpDir "requirements.txt"
    $venvDir = Join-Path $mcpDir ".venv"
    $venvPython = Join-Path $venvDir "Scripts\python.exe"

    if (-not (Test-Path -LiteralPath $serverHttpPy)) {
        throw "server_http.py not found at: $($serverHttpPy). Copy the AiDoc folder to this machine first."
    }
    ...
    if ($existingSvc) {
        ...
        $nssmPath = Join-Path $AiDocRoot "tools\nssm.exe"
    ...
    $nssmExe = Join-Path $AiDocRoot "tools\nssm.exe"
    ...
    & $nssmExe set $ServiceName AppStdout (Join-Path $AiDocRoot "logs\$($ServiceName)_stdout.log") 2>&1 | Out-Null
    & $nssmExe set $ServiceName AppStderr (Join-Path $AiDocRoot "logs\$($ServiceName)_stderr.log") 2>&1 | Out-Null
    New-Item -Path (Join-Path $AiDocRoot "logs") -ItemType Directory -Force -ErrorAction SilentlyContinue | Out-Null
