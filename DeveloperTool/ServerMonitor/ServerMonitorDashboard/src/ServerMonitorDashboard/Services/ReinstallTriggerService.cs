...
/// This allows the Dashboard service to be updated even when no user is logged in (unlike the tray app).
...
public class ReinstallTriggerService : BackgroundService
{
    ...
    // Base path for trigger files (same as agent and tray apps)
    ...
    // Dashboard-specific trigger file name
    private static readonly string GlobalTriggerPath = Path.Combine(ConfigBasePath, "ReinstallServerMonitorDashboard.txt");
    
    // Machine-specific reinstall trigger file (higher priority)
    private static readonly string MachineTriggerPath = Path.Combine(ConfigBasePath, $"ReinstallServerMonitorDashboard_{Environment.MachineName}.txt");
    ...
    {
        _logger.LogInformation("🔄 Dashboard reinstall trigger monitor started for {MachineName}", Environment.MachineName);
        ...
        _logger.LogInformation("🔄 Dashboard reinstall trigger monitor stopped");
    }
    ...
    private async Task HandleTriggerFileDetected(string triggerFilePath, string? version)
    {
        ...
        _logger.LogWarning("═══════════════════════════════════════════════════════");
        _logger.LogWarning("🔄 DASHBOARD REINSTALL TRIGGER DETECTED: {TriggerFile}", triggerFilePath);
        ...
        if (success)
        {
            ...
            _logger.LogWarning("🔄 Dashboard service will be stopped and updated by the install script");
    ...
    private bool LaunchInstallScript()
    {
        try
        {
            ...
            var installCommands = @"
                ...
                Install-OurPshApp -AppName 'ServerMonitorDashboard'
                
                # Step 2: Run the install script (this will stop/update/restart the service)
                Start-OurPshApp -AppName 'ServerMonitorDashboard'
            ";
