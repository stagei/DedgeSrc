<#
.SYNOPSIS
    Configures Chrome, Edge, and Chromium to allow automatic Kerberos/Negotiate
    authentication to DedgeAuth servers, eliminating the Windows Security credential prompt.

.DESCRIPTION
    Sets the AuthServerAllowlist registry policy for Chrome, Edge, and Chromium
    (including Playwright/Cursor IDE browser) so the browser sends Kerberos tickets
    automatically to trusted servers. Also adds the server to the Local Intranet
    zone for IE/Edge legacy compatibility.
...
$registryPaths = @(
    ...
    @{ Path = "HKLM:\SOFTWARE\Policies\Chromium";         Name = "Chromium" }
)
...
# Verify Kerberos TGT — MUST use Windows klist.exe, not Java/MIT klist
# Java JDK klist (often first in PATH) uses MIT file-based cache (krb5cc_*)
# which doesn't exist on Azure AD-joined PCs. Windows klist.exe reads the
# SSPI/LSASS credential cache where CloudAP stores the actual TGT.
$winKlist = Join-Path $env:WINDIR "System32\klist.exe"
Write-Host "`n=== Kerberos Ticket Status ===" -ForegroundColor Cyan
Write-Host "Using: $winKlist" -ForegroundColor DarkGray
try {
    $klistOutput = & $winKlist 2>&1
...
Write-Host "1. Chrome, Edge, and Chromium are now configured to send Kerberos tickets to: $Servers"
Write-Host "2. dedge-server is in the Local Intranet zone"
Write-Host "3. Restart your browser for changes to take effect"
Write-Host "4. Cursor IDE browser (Chromium-based) will also use Negotiate auth"
Write-Host "5. Click 'Sign in with Windows' on the DedgeAuth login page -- no prompt expected`n"
