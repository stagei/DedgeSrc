...
let alias = "";
function viewerDocHrefForAutoDoc(file) {
  return alias
    ? `doc.html?file=${encodeURIComponent(file)}&alias=${encodeURIComponent(alias)}`
    : `doc.html?file=${encodeURIComponent(file)}`;
}
...
function checkAutoDocAvailable(programName, defaultJsonFile) {
  if (autoDocCache[programName] !== undefined) {
    if (autoDocCache[programName]) {
      ...
      if (link && autoDocCache[programName] !== defaultJsonFile) {
        link.href = viewerDocHrefForAutoDoc(autoDocCache[programName]);
      }
    }
    return;
  }
  ...
  (async () => {
    const adQ = alias ? `?alias=${encodeURIComponent(alias)}` : "";
    for (const ext of autoDocExtensions) {
      const candidate = programName + ext;
      try {
        const r = await fetch(`api/autodoc/${encodeURIComponent(candidate)}${adQ}`, { method: "HEAD" });
        if (r.ok) {
          autoDocCache[programName] = candidate;
          const badge = document.getElementById(`autodocBadge_${programName}`);
          if (badge) badge.style.display = "inline-block";
          const link = document.querySelector(`a.autodoc-link-btn`);
          if (link) link.href = viewerDocHrefForAutoDoc(candidate);
          return;
        }
      } catch { /* ignore */ }
    }
...
function showProgramDetail(name) {
  ...
  if (adExists) {
    h += `<a class="autodoc-link-btn" href="${viewerDocHrefForAutoDoc(autoDocFile)}" target="_blank" rel="noopener">View Full Documentation</a>`;
  } else {
