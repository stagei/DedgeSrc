...
function docLink(jsonFile, label) {
  ...
  const aq = docPageAlias ? `&alias=${encodeURIComponent(docPageAlias)}` : "";
  return `<a href="doc.html?file=${encodeURIComponent(safe)}${aq}">${esc(label || safe)}</a>`;
}
...
async function fetchDoc(fileName) {
  const q = docPageAlias ? `?alias=${encodeURIComponent(docPageAlias)}` : "";
  const resp = await fetch(`api/autodoc/${encodeURIComponent(fileName)}${q}`);
  if (!resp.ok) {
