...
public static class CobdokExportService
{
    ...
    {
        AutoDocLogger.LogMessage("Exporting tables from cobdok and fkavdnt databases", LogLevel.INFO);
        ...
        AutoDocLogger.LogMessage($"Running db2cmd.exe with script: {scriptPath}", LogLevel.INFO);
        try
        {
            ...
            if (process != null)
            {
                ...
                AutoDocLogger.LogMessage($"db2cmd.exe exited with code {process.ExitCode}", LogLevel.INFO);
            }
        }
        catch (Exception ex)
        {
            AutoDocLogger.LogMessage($"db2cmd.exe failed: {ex.Message}", LogLevel.ERROR, ex);
        }
        ...
        AutoDocLogger.LogMessage($"Exported {filesToConvert.Length} metadata files to {cobdokFolder}", LogLevel.INFO);
    }
    ...
    private static void ConvertFromAnsi1252ToUtf8(string tableName, string folderPath)
    {
        ...
        {
            AutoDocLogger.LogMessage($"File {filePath} does not exist", LogLevel.WARN);
            return;
        }
        ...
        {
            AutoDocLogger.LogMessage($"Error converting {filePath} to UTF-8: {ex.Message}", LogLevel.WARN, ex);
        }
    }
}
