...
public static class ExecutionPathHelper
{
    ...
    public static bool GenerateSvgFile(string mmdFilename, string? configPath = null)
    {
        try
        {
            ...
            if (process == null)
            {
                AutoDocLogger.LogMessage("Failed to start mmdc.cmd", LogLevel.ERROR);
                return false;
            }
            ...
            {
                AutoDocLogger.LogMessage($"Error in mmdc.cmd. SVG not generated. See: {mmdFilename}", LogLevel.ERROR);
                return false;
            }
        ...
        {
            AutoDocLogger.LogMessage($"Error in GenerateSvgFile: {ex.Message}", LogLevel.ERROR, ex);
            return false;
        }
    }
}
