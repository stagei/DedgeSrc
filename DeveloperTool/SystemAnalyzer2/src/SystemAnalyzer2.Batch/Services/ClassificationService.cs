...
public sealed class ClassificationService
{
    ...
    /// <summary>Optional per-run folder (e.g. <c>_History/.../autodoc</c>) checked before <see cref="SystemAnalyzerOptions.AutoDocJsonPath"/>.</summary>
    private string? _autodocOverlayRoot;
    ...
    /// <summary>Prefer AutoDoc files from this directory (API fetch or local generation) for the current analysis run.</summary>
    public void SetAutodocOverlayDirectory(string? directory)
    {
        _autodocOverlayRoot = string.IsNullOrWhiteSpace(directory) ? null : directory.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar);
        _autoDocCache.Clear();
        _autoDocPathVerified = false;
    }

    /// <summary>
    /// Returns the file path for a program's AutoDoc JSON (overlay first when configured).
    /// </summary>
    public string GetAutoDocFilePath(string programName)
    {
        var name = $"{programName.ToUpperInvariant()}.CBL.json";
        if (!string.IsNullOrEmpty(_autodocOverlayRoot))
            return Path.Combine(_autodocOverlayRoot, name);
    ...
    public bool AutoDocFileExists(string programName)
    {
        ...
        if (!string.IsNullOrEmpty(_autodocOverlayRoot))
        {
            var o = Path.Combine(_autodocOverlayRoot, name);
            if (File.Exists(o)) return true;
        }
    ...
    public JsonObject? GetAutoDocData(string programName)
    {
        ...
        var name = $"{key}.CBL.json";
        string? filePath = null;
        if (!string.IsNullOrEmpty(_autodocOverlayRoot))
        {
            var overlay = Path.Combine(_autodocOverlayRoot, name);
            if (File.Exists(overlay)) filePath = overlay;
        }

        if (filePath == null)
        {
            if (!EnsureAutoDocPathAccessible()) return null;
            var primary = Path.Combine(_options.AutoDocJsonPath ?? "", name);
