...
using NLog;
using SystemAnalyzer2.Batch.Models;
using SystemAnalyzer2.Batch.Services;
using SystemAnalyzer2.Batch.Services.Analyzers;
using SystemAnalyzer2.Batch.Services.Analyzers.Cobol;
using SystemAnalyzer2.Batch.Services.Database;
using SystemAnalyzer2.Batch.Services.Discovery;
...
namespace SystemAnalyzer2.Batch;
...
public sealed class FullAnalysisPipeline
{
    ...
    private readonly IReadOnlyList<ITechnologyAnalyzer> _technologyAnalyzers;
    ...
    public FullAnalysisPipeline(
        ...
        IReadOnlyList<ITechnologyAnalyzer>? technologyAnalyzers = null)
    {
        _options = options;
        ...
        _technologyAnalyzers = technologyAnalyzers ?? Array.Empty<ITechnologyAnalyzer>();
    }
    ...
    /// <summary>Per-run <c>{runDir}/autodoc</c> — local CBL JSON and API overlay for <see cref="ClassificationService"/>.</summary>
    private string? _runAutodocDir;
    ...
    public async Task<int> RunAsync(AnalysisRunRequest request)
    {
        ...
        if (request.CleanBeforeRun)
        {
            Logger.Info("Cleaning published outputs for alias (root *.json/*.md + autodoc/, preserving _History; AnalysisCommon2/Databases cleared on multi-DB export)…");
            foreach (var root in EnumerateDistinctResultRoots())
                CleanPublishedAliasOutputs(root, alias);
        }
        ...
        _runAutodocDir = Path.Combine(runDir, "autodoc");
        Directory.CreateDirectory(_runAutodocDir);
        _classifier.SetAutodocOverlayDirectory(_runAutodocDir);
        ...
        var allV2 = AllJsonReader.Load(request.AllJsonPath);
        ValidateTechnologyProfile(allV2, request.AllJsonPath);

        var compatRoot = BuildCompatAllJsonRoot(allV2);
        _profileDatabase = compatRoot["database"]?.GetValue<string>() ?? "";
        ...
        await RunMultiDbDiscoveryAsync(allV2, runDir, request.CleanBeforeRun, CancellationToken.None);
        ...
        var entriesArray = compatRoot["entries"]!.AsArray();
        var seedPrograms = entriesArray
        ...
        var allNodeForTech = JsonSerializer.SerializeToNode(allV2) as JsonObject ?? new JsonObject();
        await ProcessAdditionalTechnologiesAsync(allV2, allNodeForTech, master, alias, runDir, CancellationToken.None);
        ...
        {
            if (!_options.GenerateLocally)
            {
                Logger.Info("  AutoDoc: GenerateLocally=false — downloading CBL JSON into run autodoc folder…");
                await FetchAutoDocJsonFromApiAsync(master, _runAutodocDir!, CancellationToken.None);
            }
        ...
        CrossTechLinker.Apply(master);
        var restMap = RestApiDiscovery.BuildRestApiMap(master);
        await File.WriteAllTextAsync(Path.Combine(runDir, "rest_api_map.json"), restMap.ToJsonString(JsonWriteOptions), CancellationToken.None);
        ...
        // Copy latest to each distinct results root (web reads AnalysisResultsRoot; batch history uses DataRoot)
        foreach (var root in EnumerateDistinctResultRoots())
            CopyToAliasFolder(runDir, Path.Combine(root, alias));

        UpdateAnalysesIndexAfterRun(alias, runDir, request.AllJsonPath, CollectProfileAreas());
    ...
    /// <summary>
    /// Phase 8 publish: merge this run into <c>analyses.json</c> under each distinct results root (V1 PowerShell parity).
    /// </summary>
    private IReadOnlyList<string> CollectProfileAreas()
    {
        var set = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        if (_allJsonEntries == null) return [];
        foreach (var e in _allJsonEntries)
        {
            var a = e?["area"]?.GetValue<string>();
    ...
    private void UpdateAnalysesIndexAfterRun(string alias, string runDir, string allJsonPath, IReadOnlyList<string> areas)
    {
        var roots = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        foreach (var p in new[] { _options.DataRoot, _options.AnalysisResultsRoot })
        {
            if (string.IsNullOrWhiteSpace(p)) continue;
            try
            {
                roots.Add(Path.GetFullPath(p.TrimEnd('\\', '/')));
            }
            catch
            {
                /* ignore invalid path */
        ...
        foreach (var dataRoot in roots)
        {
            try
            {
                if (!Directory.Exists(dataRoot)) continue;

                var service = new AnalysisIndexService();
                var index = service.Load(dataRoot);
                var runFolderName = Path.GetFileName(runDir.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar));
                var nowStr = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");

                var meta = index.Analyses.FirstOrDefault(a => string.Equals(a.Alias, alias, StringComparison.OrdinalIgnoreCase));
                if (meta == null)
                {
                    meta = new AnalysisMetadata { Alias = alias };
                    index.Analyses.Add(meta);
                }

                meta.LastRun = nowStr;
                meta.LatestFolder = alias;
                meta.AllJsonSourcePath = allJsonPath;
                meta.Areas = areas.Count > 0 ? [.. areas] : [];
                meta.Parameters = new AnalysisParameters
                {
                    Db2Dsn = _options.Db2Dsn ?? "",
                    MaxCallIterations = _options.MaxCallIterations,
                    RagResults = _options.RagResults,
                    RagTableResults = _options.RagTableResults
                };
                if (string.IsNullOrEmpty(meta.Created))
                    meta.Created = nowStr;

                meta.Runs ??= [];
                if (!meta.Runs.Any(r => string.Equals(r.Folder, runFolderName, StringComparison.OrdinalIgnoreCase)))
                    meta.Runs.Insert(0, new AnalysisRun { Folder = runFolderName, Timestamp = nowStr });

                service.Save(dataRoot, index);
                Logger.Info($"  analyses.json updated under {dataRoot} (alias={alias}, run={runFolderName})");
            }
            catch (Exception ex)
            {
                Logger.Warn(ex, $"  analyses.json update failed for {dataRoot} (non-fatal)");
            }
        }
    }
    ...
    private async Task ExtractProgramAsync(string program, JsonObject master)
    {
        ...
        {
            TryGenerateLocalCblAutodoc(norm, resolution.Path);
            try
            {
        ...
        if (cachedExtraction != null)
        {
            ...
            if (cacheValid)
            {
                var entry = new JsonObject
                {
                    ...
                    ["technology"] = cachedExtraction["technology"]?.GetValue<string>() ?? "cobol",
                    ...
                    ["vsamFiles"] = cachedExtraction["vsamFiles"]?.DeepClone() ?? new JsonArray(),
                };
        ...
        if (!string.IsNullOrEmpty(text))
        {
            ...
            newEntry["vsamFiles"] = JsonSerializer.SerializeToNode(CobolAnalyzerBase.ExtractVsamFiles(text));
        }
        else
        {
            ...
            newEntry["vsamFiles"] = new JsonArray();
        }
        ...
        _cacheService.SaveCachedFact(norm, "extraction", new JsonObject
        {
            ...
            ["actualName"] = resolution.ActualName,
            ["program"] = norm,
            ["technology"] = newEntry["technology"]?.DeepClone(),
            ...
            ["vsamFiles"] = newEntry["vsamFiles"]?.DeepClone() ?? new JsonArray(),
    ...
    /// <summary>
    /// When <see cref="SystemAnalyzerOptions.GenerateLocally"/> is true, parse each local <c>.cbl</c> into
    /// <see cref="_runAutodocDir"/> as <c>{PROG}.CBL.json</c> for classification overlay.
    /// </summary>
    private void TryGenerateLocalCblAutodoc(string programUpper, string sourceFilePath)
    {
        if (!_options.GenerateLocally || string.IsNullOrEmpty(_runAutodocDir)) return;
        if (string.IsNullOrEmpty(sourceFilePath) || !sourceFilePath.EndsWith(".cbl", StringComparison.OrdinalIgnoreCase))
            return;
        if (!File.Exists(sourceFilePath)) return;

        var desiredFileName = $"{programUpper}.CBL.json";
        if (TryFindAutodocJsonFile(_runAutodocDir, desiredFileName, out var existingPath))
        {
            if (string.Equals(Path.GetFileName(existingPath), desiredFileName, StringComparison.Ordinal))
                return;
            try
            {
                var target = Path.Combine(_runAutodocDir, desiredFileName);
                MoveOrCopyAutodocJsonToCanonical(existingPath, target, _runAutodocDir);
            }
            catch (Exception ex)
            {
                Logger.Debug(ex, "Autodoc rename to {Name} skipped", desiredFileName);
            }
        ...
        try
        {
            var tmpRoot = Path.Combine(_runAutodocDir, "_cbltmp");
            Directory.CreateDirectory(tmpRoot);
            var srcRoot = _options.SourceRoot ?? "";
            CblParser.StartCblParse(
                sourceFilePath,
                show: false,
                outputFolder: _runAutodocDir,
                cleanUp: true,
                tmpRootFolder: tmpRoot,
                srcRootFolder: srcRoot,
                clientSideRender: false,
                saveMmdFiles: false,
                generateHtml: false);
            EnsureAutodocJsonCanonicalName(programUpper, sourceFilePath);
        }
        catch (Exception ex)
        {
            Logger.Debug(ex, "Local CblParser failed for {Prog}", programUpper);
        }
    }

    /// <summary>Find <c>*.json</c> in <paramref name="dir"/> whose name equals <paramref name="desiredFileName"/> (case-insensitive).</summary>
    private static bool TryFindAutodocJsonFile(string dir, string desiredFileName, out string fullPath)
    {
        fullPath = "";
        if (!Directory.Exists(dir)) return false;
        foreach (var f in Directory.EnumerateFiles(dir, "*.json", SearchOption.TopDirectoryOnly))
        {
            if (string.Equals(Path.GetFileName(f), desiredFileName, StringComparison.OrdinalIgnoreCase))
            {
                fullPath = f;
                return true;
            }
    ...
    /// <summary>
    /// Parser writes <c>{diskFileName}.json</c> (e.g. <c>prog.cbl.json</c>); overlay lookup uses <c>PROG.CBL.json</c>.
    /// </summary>
    private void EnsureAutodocJsonCanonicalName(string programUpper, string sourceFilePath)
    {
        if (string.IsNullOrEmpty(_runAutodocDir)) return;
        var desiredFileName = $"{programUpper}.CBL.json";
        var canonical = Path.Combine(_runAutodocDir, desiredFileName);
        var produced = Path.Combine(_runAutodocDir, Path.GetFileName(sourceFilePath) + ".json");
        if (!File.Exists(produced)) return;
        if (string.Equals(Path.GetFileName(produced), desiredFileName, StringComparison.Ordinal)) return;
        MoveOrCopyAutodocJsonToCanonical(produced, canonical, _runAutodocDir);
    }

    /// <summary>Windows: case-only renames need two-step move via a temp file in <paramref name="autodocDir"/>.</summary>
    private static void MoveOrCopyAutodocJsonToCanonical(string sourcePath, string targetPath, string autodocDir)
    {
        if (string.Equals(Path.GetFullPath(sourcePath), Path.GetFullPath(targetPath), StringComparison.OrdinalIgnoreCase))
        {
            var tmp = Path.Combine(autodocDir, ".cblrename_" + Guid.NewGuid().ToString("N") + ".json");
            File.Move(sourcePath, tmp);
    ...
    private IEnumerable<string> EnumerateDistinctResultRoots()
    {
        var set = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        foreach (var p in new[] { _options.DataRoot, _options.AnalysisResultsRoot })
        {
            ...
            try
            {
                set.Add(Path.GetFullPath(p.TrimEnd('\\', '/')));
            }
            catch
            {
                /* ignore invalid path */
    ...
    /// <summary>
    /// Removes published flat outputs for an alias (preserves <c>_History</c>). Used before a clean run.
    /// </summary>
    private static void CleanPublishedAliasOutputs(string dataRoot, string alias)
    {
        var safe = JsonDataService.SanitizeAlias(alias);
        if (string.IsNullOrEmpty(safe)) return;
        var aliasDir = Path.Combine(dataRoot, safe);
        if (!Directory.Exists(aliasDir)) return;

        foreach (var file in Directory.GetFiles(aliasDir))
        {
            try
            {
                File.Delete(file);
            ...
            {
                Logger.Warn($"  Clean: could not delete {file}: {ex.Message}");
            }
        }

        var autodoc = Path.Combine(aliasDir, "autodoc");
        if (Directory.Exists(autodoc))
        {
            try
            {
                Directory.Delete(autodoc, true);
            }
            catch (Exception ex)
            {
                Logger.Warn($"  Clean: could not remove autodoc dir: {ex.Message}");
            }
        }
    }
    ...
    private void CopyToAliasFolder(string runDir, string aliasDir)
    {
        ...
        var autodocSrc = Path.Combine(runDir, "autodoc");
        if (Directory.Exists(autodocSrc))
        {
            var autodocDest = Path.Combine(aliasDir, "autodoc");
            Directory.CreateDirectory(autodocDest);
            foreach (var file in Directory.GetFiles(autodocSrc, "*.json", SearchOption.AllDirectories))
            {
                var rel = Path.GetRelativePath(autodocSrc, file);
                var destFile = Path.Combine(autodocDest, rel);
    ...
    /// <summary>GET <c>{ResolveAutoDocFetchBaseUrl}/docs/{PROG}.CBL.json</c> into the run <paramref name="autodocDir"/>.</summary>
    private async Task FetchAutoDocJsonFromApiAsync(JsonObject master, string autodocDir, CancellationToken ct)
    {
        var baseUrl = _options.ResolveAutoDocFetchBaseUrl();
        if (string.IsNullOrEmpty(baseUrl))
        {
            Logger.Warn("  AutoDoc API: no fetch base URL (set AutoDocJsonApiUrl or AutoDocApiUrl) — skip remote fetch.");
            return;
        }
        ...
        foreach (var p in master["programs"]!.AsArray())
        {
            ...
            if (!tech.Equals("cobol", StringComparison.OrdinalIgnoreCase))
                continue;

            var name = p!["program"]!.GetValue<string>().ToUpperInvariant();
            var dest = Path.Combine(autodocDir, $"{name}.CBL.json");
            if (File.Exists(dest))
            {
                ok++;
                continue;
            ...
            foreach (var rel in new[] { $"docs/{name}.CBL.json", $"docs/{name}.cbl.json" })
            {
                var url = $"{baseUrl}/{rel}";
                try
                {
                ...
                {
                    Logger.Debug($"  AutoDoc fetch {url}: {ex.Message}");
                }
            }

            if (!File.Exists(dest)) fail++;
        }

        Logger.Info($"  AutoDoc API fetch: {ok} files present, {fail} programs without remote JSON");
    }
    ...
    private async Task RunMultiDbDiscoveryAsync(AllJsonV2 allV2, string runDir, bool cleanCommon2Shared, CancellationToken ct)
    {
        if (allV2.Databases.Count == 0) return;
        var dbRoot = Path.Combine(runDir, "Databases");
        string? common2Db = null;
        if (!string.IsNullOrWhiteSpace(_options.AnalysisCommon2Path))
            common2Db = Path.Combine(_options.AnalysisCommon2Path.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar), "Databases");

        if (cleanCommon2Shared && !string.IsNullOrEmpty(common2Db) && Directory.Exists(common2Db))
        {
            foreach (var f in Directory.GetFiles(common2Db, "*.json", SearchOption.TopDirectoryOnly))
            {
                try
                {
                    File.Delete(f);
                ...
                {
                    Logger.Warn($"  Clean AnalysisCommon2: {f}: {ex.Message}");
                }
            }

            Logger.Info("  Cleaned AnalysisCommon2/Databases (*.json) before multi-DB export");
        }

        foreach (var d in allV2.Databases)
        {
            var kind = (d.Type ?? "").ToLowerInvariant();
            IDatabaseDiscovery? disc = kind switch
            {
                "postgresql" or "postgres" => new PostgreSqlDiscovery(d.ConnectionName ?? d.Database ?? "default"),
                "sqlserver" or "mssql" or "azure-sql" => new SqlServerDiscovery(d.ConnectionName ?? d.Alias ?? "default"),
                "sqlite" => new SqliteDiscovery(d.Alias ?? "sqlite", d.Database ?? ""),
                _ => null
            };
            if (disc == null) continue;
            await disc.ExportCatalogAsync(dbRoot, ct);
            if (!string.IsNullOrEmpty(common2Db))
                await disc.ExportCatalogAsync(common2Db, ct);
        }
    }

    private async Task ProcessAdditionalTechnologiesAsync(
        AllJsonV2 allV2, JsonObject allNode, JsonObject master, string alias, string runDir, CancellationToken ct)
    {
        foreach (var kv in allV2.Technologies)
        {
            if (kv.Key.Equals("cobol", StringComparison.OrdinalIgnoreCase)) continue;
            var section = kv.Value;
            if (section.Entries.Count == 0) continue;

            var cfg = AllJsonReader.ToConfig(kv.Key, section);
            var analyzer = _technologyAnalyzers.FirstOrDefault(a => a.CanHandle(cfg));
            if (analyzer == null)
            {
                Logger.Warn($"  Skipping technology '{kv.Key}' — no registered analyzer for vendor={cfg.Vendor} product={cfg.Product}");
                continue;
            }

            try
            {
                var treq = new TechAnalysisRequest(alias, runDir, allNode, section.Entries, _options, cfg);
                var res = await analyzer.AnalyzeAsync(treq, ct);
                foreach (var p in res.Programs)
                    master["programs"]!.AsArray().Add(p);
                if (res.Programs.Count > 0)
                    Logger.Info($"  Technology '{kv.Key}': merged {res.Programs.Count} nodes into dependency master");
            }
            catch (NotImplementedException ex)
            {
                Logger.Warn($"  Technology '{kv.Key}': {ex.Message}");
            }
        }
    }

    private static JsonObject BuildCompatAllJsonRoot(AllJsonV2 allV2)
    {
        var root = new JsonObject();
        _ = allV2.Technologies.TryGetValue("cobol", out var cobol);
        root["database"] = cobol?.Database ?? allV2.Database ?? "";
        var entriesArr = new JsonArray();
        if (cobol is { Entries.Count: > 0 })
        {
            foreach (var e in cobol.Entries)
                entriesArr.Add(e.DeepClone());
        }
        else if (allV2.Entries is { Count: > 0 })
        {
            foreach (var e in allV2.Entries)
                entriesArr.Add(e.DeepClone());
        }
    ...
    private void ValidateTechnologyProfile(AllJsonV2 allV2, string allJsonPath)
    {
        if (TryLoadSupportedTechnologiesJson() is null)
            Logger.Warn("  supported-technologies.json not found under AnalysisStatic (optional validation skipped)");

        foreach (var kv in allV2.Technologies)
        {
            if (kv.Value.Entries.Count == 0) continue;
            var cfg = AllJsonReader.ToConfig(kv.Key, kv.Value);
            var analyzer = _technologyAnalyzers.FirstOrDefault(a => a.CanHandle(cfg));
            if (analyzer == null)
                Logger.Warn($"  Technology '{kv.Key}': no registered analyzer for vendor={cfg.Vendor} product={cfg.Product} ({allJsonPath})");
        }
    }

    private static JsonNode? TryLoadSupportedTechnologiesJson()
    {
        foreach (var p in GetSupportedTechnologyJsonPaths())
        {
            if (File.Exists(p))
    ...
    private static IEnumerable<string> GetSupportedTechnologyJsonPaths()
    {
        yield return Path.Combine(AppContext.BaseDirectory, "AnalysisStatic", "supported-technologies.json");
        yield return Path.Combine(Directory.GetCurrentDirectory(), "AnalysisStatic", "supported-technologies.json");
    }
