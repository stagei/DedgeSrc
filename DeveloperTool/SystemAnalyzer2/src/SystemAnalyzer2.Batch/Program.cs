...
public class Program
{
    ...
    public static async Task<int> Main(string[] args)
    {
        ...
        Logger.Info("SystemAnalyzer2.Batch starting");
        ...
        Logger.Info($"  GenerateLocally: {options.GenerateLocally}");
        Logger.Info($"  AutoDoc fetch:   {options.ResolveAutoDocFetchBaseUrl()}");
        Logger.Info($"  Clean before run: {request.CleanBeforeRun}");
        ...
        IReadOnlyList<ITechnologyAnalyzer> technologyAnalyzers =
        [
            new RocketVisualCobolAnalyzer(),
        ...
        var pipeline = new FullAnalysisPipeline(
            ...
            catalogExportService, namingService, httpClient, technologyAnalyzers);
        ...
        for (int i = 0; i < args.Length; i++)
        {
            switch (args[i].ToLowerInvariant())
            {
                ...
                case "--generate-stats":
                    generateStats = true;
                    break;
                case "--no-generate-locally":
                    options.GenerateLocally = false;
                    break;
                case "--generate-locally":
                    options.GenerateLocally = true;
                    break;
                case "--autodoc-api-url" when i + 1 < args.Length:
                    options.AutoDocApiUrl = args[++i];
                    break;
                case "--no-clean-before-run":
    ...
    {
        Console.WriteLine("Usage: SystemAnalyzer2.Batch --all-json <path> [--alias <name>] [--skip-phases 5,6] [--skip-classification] [--skip-naming] [--skip-catalog] [--refresh-catalogs] [--generate-stats] [--no-clean-before-run] [--no-generate-locally] [--autodoc-api-url <url>]");
    }
}
