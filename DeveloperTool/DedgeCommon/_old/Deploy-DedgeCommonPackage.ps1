...
function Set-ProjectVersion {
    ...
    Write-Host "✓ Updated project version to $NewVersion" -ForegroundColor Green
}
...
if (-not $SkipTests) {
    ...
    try {
        ...
        if ($testResult) {
            Write-Host "✓ VerifyFunctionality test passed (7/7)" -ForegroundColor Green
        }
        else {
            Write-Host "⚠ VerifyFunctionality test results unclear" -ForegroundColor Yellow
        }
    }
    catch {
...
Write-Host "✓ Package created: Dedge.DedgeCommon.$newVersion.nupkg" -ForegroundColor Green
...
if ([string]::IsNullOrEmpty($PAT)) {
    # Try to load PAT from config file in OneDrive Documents
    $configPath = Join-Path $env:OneDriveCommercial "Documents\DedgeCommonNuget.json"
    
    if (Test-Path $configPath) {
        Write-Host "Loading PAT from config file: $configPath" -ForegroundColor Gray
        try {
            $config = Get-Content $configPath -Raw | ConvertFrom-Json
            $PAT = $config.PAT
            
            if (-not [string]::IsNullOrEmpty($PAT)) {
                Write-Host "OK PAT loaded from config file" -ForegroundColor Green
                Write-Host "  Email: $($config.Email)" -ForegroundColor Gray
                Write-Host "  Last Updated: $($config.LastUpdated)" -ForegroundColor Gray
            }
            else {
                Write-Host "Warning: PAT field empty in config file" -ForegroundColor Yellow
            }
        }
        catch {
    ...
    if ([string]::IsNullOrEmpty($PAT)) {
        ...
        if ([string]::IsNullOrEmpty($PAT)) {
            ...
            Write-Host ""
            Write-Host "Or update config file: $configPath" -ForegroundColor Cyan
            exit 0
        }
    }
}
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        if ($pushResult -like "*409*" -or $pushResult -like "*already exists*") {
            Write-Host "⚠ Package version $newVersion already exists in feed" -ForegroundColor Yellow
    ...
    Write-Host "✗ Error pushing package: $($_.Exception.Message)" -ForegroundColor Red
    $deployed = $false
}
...
if ($deployed) {
    Write-Host "Deploy Status:    ✓ Deployed to Dedge feed" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "⚠ Package built but not deployed" -ForegroundColor Yellow
    Write-Host ""
