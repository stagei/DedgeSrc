...
public class AiExportService
{
    ...
    public async Task<string> ExportToMarkdownAsync(string content, string title, string? outputPath = null)
    {
        ...
        if (string.IsNullOrWhiteSpace(outputPath))
        {
            ...
            outputPath = Path.Combine(Path.GetTempPath(), "DbExplorer", "AI_Exports", fileName);
        }
        ...
        markdown.AppendLine($"tool: DbExplorer AI Assistant");
        markdown.AppendLine("---");
        markdown.AppendLine();
        ...
        markdown.AppendLine("---");
        markdown.AppendLine();
        markdown.AppendLine("*Generated by DbExplorer AI Assistant*");
    ...
    public async Task<string> ExportMermaidWithContextAsync(string mermaidDiagram, string contextMarkdown, string title, string? outputPath = null)
    {
        ...
        if (string.IsNullOrWhiteSpace(outputPath))
        {
            var timestamp = DateTime.Now.ToString("yyyyMMdd_HHmmss");
            var safeTitle = SanitizeFileName(title);
            outputPath = Path.Combine(Path.GetTempPath(), "DbExplorer", "AI_Exports", $"Mermaid_{safeTitle}_{timestamp}.md");
        }
    ...
    public async Task<string> ExportSqlAsync(string sql, string title, string? outputPath = null)
    {
        ...
        content.AppendLine($"-- Tool: DbExplorer");
        content.AppendLine();
