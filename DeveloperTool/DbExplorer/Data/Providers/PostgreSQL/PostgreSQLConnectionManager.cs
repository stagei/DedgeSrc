...
namespace DbExplorer.Data.Providers.PostgreSQL;

/// <summary>
/// Manager for PostgreSQL database connections.
/// Uses Npgsql for PostgreSQL connectivity. Implements IConnectionManager for database-agnostic usage.
/// </summary>
public class PostgreSQLConnectionManager : IConnectionManager
{
    ...
    public PostgreSQLConnectionManager(DatabaseConnection connectionInfo)
    {
        Logger.Debug("PostgreSQLConnectionManager initializing with Npgsql driver");
    ...
    public async Task<bool> TestConnectionAsync()
    {
        ...
        Logger.Debug("Server: {Server}, Database: {Database}", _connectionInfo.Server, _connectionInfo.Database);

        try
        {
            await using var testConnection = new NpgsqlConnection(_connectionInfo.GetConnectionString());
        ...
        {
            Logger.Error(pgEx, "PostgreSQL connection test failed - Code: {Code}, SqlState: {SqlState}",
    ...
    public async Task OpenAsync(CancellationToken cancellationToken = default)
    {
        ...
        Logger.Debug("Connection string: {Connection}", _connectionInfo.GetConnectionString(maskPassword: true));

        try
        {
            _connection = new NpgsqlConnection(_connectionInfo.GetConnectionString());
            cancellationToken.ThrowIfCancellationRequested();
            await _connection.OpenAsync(cancellationToken);
            cancellationToken.ThrowIfCancellationRequested();

            Logger.Info("PostgreSQL connection opened successfully - State: {State}", _connection.State);
            Logger.Debug("PostgreSQL Server Version: {Version}", _connection.PostgreSqlVersion);
        ...
        {
            Logger.Error(pgEx, "Failed to open PostgreSQL connection - SqlState: {SqlState}", pgEx.SqlState);
            throw new Exception($"Failed to connect to database: {pgEx.Message}", pgEx);
        }
        catch (Exception ex)
        {
            Logger.Error(ex, "Failed to open PostgreSQL connection");
    ...
    public async Task<DataTable> ExecuteQueryAsync(string sql, int? maxRows = null, int offset = 0, bool handleDecimalErrors = true)
    {
        ...
        if (_connectionInfo.Permissions != null && IsModifyingSql(sql))
        {
            ...
            {
                Logger.Warn("LOW level user {Username} attempted to execute modifying SQL", _connectionInfo.Permissions.Username);
            ...
            if (accessLevel == UserAccessLevel.Advanced)
            {
                ...
                {
                    Logger.Warn("MIDDLE level user {Username} attempted to execute DDL", _connectionInfo.Permissions.Username);
                    throw new UnauthorizedAccessException(
                        $"Access Denied: You have STANDARD USER access ({_connectionInfo.Permissions.AccessLevelBadge}). DDL requires DBA privileges.");
                }
            }
        }
        ...
        {
            Logger.Error(pgEx, "PostgreSQL query execution failed - SqlState: {SqlState}", pgEx.SqlState);
            SqlQueryHistoryService.Track(sql, source: _connectionInfo?.GetDisplayName() ?? "Unknown",
                success: false, errorMessage: pgEx.Message);
            throw new Exception($"Query failed: {pgEx.Message}", pgEx);
        }
        catch (Exception ex)
        {
    ...
    public async Task<int> ExecuteNonQueryAsync(string sql)
    {
        ...
        {
            Logger.Error(pgEx, "PostgreSQL command execution failed - SqlState: {SqlState}", pgEx.SqlState);
            throw new Exception($"Command failed: {pgEx.Message}", pgEx);
        }
        catch (Exception ex)
        {
    ...
    public async Task<object?> ExecuteScalarAsync(string sql)
    {
        ...
        {
            Logger.Error(pgEx, "PostgreSQL scalar query failed - SqlState: {SqlState}", pgEx.SqlState);
            throw new Exception($"Scalar query failed: {pgEx.Message}", pgEx);
        }
    }
    ...
    /// <summary>Determine user's access level based on pg_roles (superuser = DBA).</summary>
    private async Task DetermineUserAccessLevelAsync()
    {
        Logger.Debug("Determining user access level (PostgreSQL)");
        try
        {
            var accessControlService = new AccessControlService();
            _connectionInfo.Permissions = await accessControlService.DetermineAccessLevelAsync(this, _connectionInfo.Username ?? string.Empty);
            Logger.Info("User {Username} access level determined: {Level}",
                _connectionInfo.Username, _connectionInfo.Permissions.AccessLevel);
        }
        catch (Exception ex)
        {
            ...
            {
                Username = _connectionInfo.Username ?? string.Empty,
    ...
    private Task SetAutoCommitModeAsync(bool autoCommit)
    {
        ...
        // Npgsql uses default autocommit; transaction control is per-command. No SQL needed at open.
        Logger.Info("Auto-commit mode: {Mode}", autoCommit ? "ON" : "OFF");
        return Task.CompletedTask;
    }
    ...
    {
        if (_disposed) return;
        Logger.Debug("Disposing PostgreSQLConnectionManager");
        Close();
        _connection?.Dispose();
        _disposed = true;
    }
    ...
    /// <summary>Create a database command.</summary>
    public DbCommand CreateCommand(string sql) => ((IConnectionManager)this).CreateCommand(sql);
    ...
    {
        if (command is not NpgsqlCommand npgCommand)
            throw new ArgumentException("Command must be NpgsqlCommand", nameof(command));
        return new NpgsqlDataAdapter(npgCommand);
    }
    ...
    /// <summary>Get list of schemas.</summary>
    ...
    /// <summary>Get list of stored procedures.</summary>
    public async Task<List<string>> GetStoredProceduresAsync(string? schema = null)
    {
        var metadataHandler = App.MetadataHandler ?? throw new InvalidOperationException("MetadataHandler not initialized");
        string sql;
        if (schema != null)
            sql = metadataHandler.GetRequiredStatement("GetProceduresForSchemaSimple").Replace("?", $"'{schema}'");
        else
    ...
    /// <summary>Get server version.</summary>
    public string GetServerVersion()
    {
        if (_connection == null) return "Not connected";
        try
        {
            return _connection.PostgreSqlVersion?.ToString() ?? _connection.ServerVersion ?? "Unknown";
        }
        catch (Exception ex)
        {
