...
.DESCRIPTION
    Automatically detects SqlMermaidErdTools.exe or Python runtime and converts SQL files to Mermaid format.
...
# Function to find SqlMermaidErdTools.exe
function Find-SqlMermaidErdTools {
    # Search patterns
    $searchPaths = @(
        "samples\SqlMermaidErdTools.Samples\bin\Release\net10.0\SqlMermaidErdTools.Samples.exe",
        "samples\SqlMermaidErdTools.Samples\bin\Debug\net10.0\SqlMermaidErdTools.Samples.exe"
    )
    ...
    $found = Get-ChildItem -Path . -Recurse -Filter "SqlMermaidErdTools.Samples.exe" -ErrorAction SilentlyContinue | 
...
function Find-PythonRuntime {
    ...
    $bundledPython = "src\SqlMermaidErdTools\runtimes\win-x64\python\python.exe"
    ...
    # Check build output
    if (Test-Path "src\SqlMermaidErdTools\bin\Release\net10.0\runtimes\win-x64\python\python.exe") {
        $python = Resolve-Path "src\SqlMermaidErdTools\bin\Release\net10.0\runtimes\win-x64\python\python.exe"
        $script = Resolve-Path "src\SqlMermaidErdTools\bin\Release\net10.0\runtimes\win-x64\scripts\sql_to_mmd.py"
        return @{
            Python = $python
            Script = $script
        }
...
# Try to find SqlMermaidErdTools.exe first (preferred method)
Write-Host "`nSearching for converter..." -ForegroundColor Yellow

$converterExe = Find-SqlMermaidErdTools

if ($converterExe) {
    Write-Host "✓ Found SqlMermaidErdTools.exe" -ForegroundColor Green
    ...
    # Fallback to Python
    Write-Host "✓ SqlMermaidErdTools.exe not found, using Python fallback..." -ForegroundColor Yellow
    
    $pythonRuntime = Find-PythonRuntime
    
    if (-not $pythonRuntime) {
        Write-Host "✗ Neither SqlMermaidErdTools.exe nor Python runtime found" -ForegroundColor Red
