# Publishing SqlMermaidErdTools to NuGet.org
...
2. **API Key**
   ...
   - Click "Create"
   - Give it a name like "SqlMermaidErdTools Publishing"
   - Select expiration (365 days recommended, or never expire)
   - **Glob Pattern**: `SqlMermaidErdTools*` (to restrict to your package)
...
3. **GitHub Repository** (Optional but recommended)
   ...
   - Repository name: `SqlMermaidErdTools`
...
git remote add origin https://github.com/geirtul/SqlMermaidErdTools.git
...
This creates:
- `nupkg/SqlMermaidErdTools.0.1.0.nupkg` - Main package
- `nupkg/SqlMermaidErdTools.0.1.0.snupkg` - Symbol package (for debugging)

### 3. Test the Package Locally (Optional)

```powershell
# Create a test project
dotnet new console -n TestSqlMermaidErdTools
cd TestSqlMermaidErdTools

# Add local package source
dotnet nuget add source D:\opt\src\SqlMermaidErdTools\nupkg -n LocalTest

# Install your package
dotnet add package SqlMermaidErdTools --version 0.1.0

# Test it works
# (modify Program.cs to use SqlMermaidErdTools)
```
...
```powershell
dotnet nuget push nupkg/SqlMermaidErdTools.0.1.0.nupkg `
...
1. Go to https://www.nuget.org/packages/SqlMermaidErdTools
...
- Consider creating platform-specific packages:
  - `SqlMermaidErdTools.Runtime.Windows`
  - `SqlMermaidErdTools.Runtime.Linux`
  - `SqlMermaidErdTools.Runtime.MacOS`
