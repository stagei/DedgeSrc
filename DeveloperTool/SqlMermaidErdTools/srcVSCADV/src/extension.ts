...
import { SplitEditorProvider } from './editors/splitEditorProvider';
...
export function activate(context: vscode.ExtensionContext) {
    console.log('SqlMermaid ERD Tools Advanced extension is now active!');
    ...
    // Register custom split editor
    context.subscriptions.push(SplitEditorProvider.register(context));
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('sqlmermaid.openSplitEditor', async () => {
            await handleOpenSplitEditor();
        })
    );

    context.subscriptions.push(
        vscode.commands.registerCommand('sqlmermaid.toggleMode', async () => {
            vscode.window.showInformationMessage('Toggle mode is handled within the split editor');
        })
    );

    context.subscriptions.push(
        vscode.commands.registerCommand('sqlmermaid.togglePreview', async () => {
            vscode.window.showInformationMessage('Toggle preview is handled within the split editor');
        })
    );

    context.subscriptions.push(
        vscode.commands.registerCommand('sqlmermaid.selectDialect', async () => {
            vscode.window.showInformationMessage('Dialect selection is handled within the split editor');
        })
    );
...
async function handleOpenSplitEditor() {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        vscode.window.showErrorMessage('No active editor found');
        return;
    ...
    if (!ext.endsWith('.sql') && !ext.endsWith('.mmd') && !ext.endsWith('.mermaid')) {
        vscode.window.showWarningMessage('Please open a .sql or .mmd file first');
        return;
    }

    // Open with custom editor
    await vscode.commands.executeCommand('vscode.openWith', document.uri, 'sqlmermaid.splitEditor');
}

function showWelcomeMessage(context: vscode.ExtensionContext) {
    const message = '🎉 Welcome to SQL ↔ Mermaid ERD Tools Advanced! Experience the split-view editor with live preview.';
    const actions = ['Open Split Editor', 'View Commands', 'Settings', 'Dismiss'];
    
    vscode.window.showInformationMessage(message, ...actions).then(selection => {
        if (selection === 'Open Split Editor') {
            vscode.commands.executeCommand('sqlmermaid.openSplitEditor');
...
export function deactivate() {
    console.log('SqlMermaid ERD Tools Advanced extension is now deactivated');
}
