...
public class SqlToMmdConverter : BaseConverter, ISqlToMmdConverter
{
    ...
    public async Task<string> ConvertAsync(string sqlDdl, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            await ExportToFileAsync(sqlDdl, "SqlToMmd-In_Original.sql", "Original SQL input", cancellationToken);
            
            // Clean SQL to remove T-SQL brackets
            var cleanedSql = CleanSqlBrackets(sqlDdl);
            
            // Export cleaned SQL that will be sent to SQLGlot
            await ExportToFileAsync(cleanedSql, "SqlToMmd-In_Cleaned.sql", "Cleaned SQL (brackets removed)", cancellationToken);
