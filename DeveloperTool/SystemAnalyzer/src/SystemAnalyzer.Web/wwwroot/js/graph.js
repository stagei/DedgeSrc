...
class FilterPanel {
  ...
  _defaultState() {
    return {
      ...
      selections: {
        elementTypes: { programs: true, tables: false, inputFiles: false, outputFiles: false, callLinks: true, copyElements: false },
...
async function buildGraph() {
  ...
  if (elTypes.programs !== false && programSource.length) {
    programSource.forEach(mp => {
      ...
      let color = NODE_COLORS.program, nodeType = "program";
      if (p?.isDeprecated) { color = NODE_COLORS.programDeprecated; nodeType = "program-deprecated"; }
      else if (p?.source === "call-expansion") { color = NODE_COLORS.programCall; nodeType = "program-call"; }
      ...
      nodes.push({ key: name, text: displayText, type: nodeType, color: baColor || color, figure: iconProfile.program, progCategory: classIndex[name]?.classification || null, businessArea: ba || null });
...
function renderGoJS() {
  ...
  diagram.nodeTemplate = new go.Node("Auto", {
    ...
    new go.TextBlock({ margin: 8, font: "12px Segoe UI", stroke: "#e2e4ea", maxSize: new go.Size(160, NaN), wrap: go.Wrap.Fit }).bind("text", "text")
  );
...
async function renderMermaidGraph() {
  ...
  const shapeMap = { "program": "[[", "program-call": "[[", "program-deprecated": "[[", "table": "[(", "input-file": "[/", "output-file": "[/", "copy": "[" };
  const shapeCloseMap = { "program": "]]", "program-call": "]]", "program-deprecated": "]]", "table": ")]", "input-file": "/]", "output-file": "/]", "copy": "]" };
  ...
  const classAssign = { prog: [], progCall: [], progDepr: [], tbl: [], fileIn: [], fileOut: [], cp: [] };
  currentNodes.forEach(n => {
    const id = mmdId(n.key);
...
async function expandProgramToFlowchart(programName) {
  ...
  let doc;
  try {
    doc = await fetch(`api/autodoc/${encodeURIComponent(fileName)}`).then(r => r.json());
...
async function showProgramFlowchart(programName) {
  ...
  const fileName = await resolveAutoDocFile(programName);
  if (!fileName) {
    container.innerHTML = '<div style="padding:40px;text-align:center;color:var(--text2)">No AutoDocJson documentation found for ' + esc(programName) + '.<br><small style="opacity:.6">Files are served from the path configured in AutoDocJsonPath (appsettings.json).</small></div>';
    return;
  }

  try {
    const doc = await fetch(`api/autodoc/${encodeURIComponent(fileName)}`).then(r => r.json());
    const flowMmd = doc?.diagrams?.flowMmd;
    if (!flowMmd) {
